...
- **Compact-mode CSS** — `body.gr-compact` rules in [`resources/css/filament/admin/theme.css`](resources/css/filament/admin/theme.css) (appended section): collapse the sidebar out of flow (`position:fixed; width:0`) so content goes full-width, hide `.fi-sidebar-nav`/`.fi-sidebar-header`, **re-anchor the "dynamic island" footer to a fixed top-right account pill** (keeps user menu + notifications reachable), hide the global QR float and breadcrumbs. These selectors are no-ops on the phone top-nav DOM (no `.fi-sidebar`), so phones are unaffected. The bottom tab bar (`.gr-bottombar`, same blade) is shown via `body.gr-compact` (was a `@media max-width:767px`) and is the **only** portrait navigation — its "More" menu was expanded to cover all destinations (Finance, Computers, Promotions, Maintenance, Settings, …) since there is no off-canvas drawer.
...
- **Only completed/cancelled rentals are locked from editing**: `Rental::canBeEdited()` returns **true** for `QUOTATION` / `CONFIRMED` / `LATE_PICKUP` / `ACTIVE` / `LATE_RETURN` / `PARTIAL_RETURN`, and **false** only for `COMPLETED` / `CANCELLED`. `EditRental::mount()` redirects away when false. `Rental::reopenFromCompleted()` flips `COMPLETED → ACTIVE` precisely to re-enable editing + a redo of the return.
