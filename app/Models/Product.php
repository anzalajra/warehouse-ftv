...
class Product extends Model
{
    ...
    public function getAvailabilityCalendar(): array
    {
        ...
        $rentals = RentalItem::whereIn('product_unit_id', $unitIds)
            ->whereHas('rental', function ($query) {
                $query->whereIn('status', \App\Services\RentalOccupancyService::BLOCKING_STATUSES)
                    ->where('start_date', '<=', now()->addYear());
            })
            ->with(['rental' => function ($query) {
                $query->select('id', 'status', 'start_date', 'end_date');
            }, 'productUnit.kits', 'deliveryItems.delivery'])
            ->get();
        ...
        foreach ($rentals as $item) {
            ...
            $rentalEnd = (\App\Services\RentalOccupancyService::occupiedUntil($item) ?? now()->addYear())
                ->copy()->addHours($bufferHours);
            if ($rentalEnd->lt(now()->startOfDay())) {
                continue;
            }
    ...
    public function findAvailableUnits($startDate, $endDate)
    {
        ...
        $booked = \App\Services\RentalOccupancyService::bookedUnitIds($startDate, $endDate);
        $related = \App\Models\UnitKit::whereIn('unit_id', $booked)->pluck('linked_unit_id')
            ->merge(\App\Models\UnitKit::whereIn('linked_unit_id', $booked)->pluck('unit_id'))
            ->filter()->all();

        return $this->units()
            ->whereNotIn('status', [ProductUnit::STATUS_MAINTENANCE, ProductUnit::STATUS_RETIRED])
            ->whereNotIn('condition', ['broken', 'lost'])
            ->whereNotIn('id', array_unique(array_merge($booked, $related)))
            ->get();
    }
