<x-filament-panels::page>
    @php
        ...
        $canValidate = $allChecked && $availability['available'];
        ...
        // Currently edited item (editor modal)
    ...
    <div id="op-console" data-density="comfortable" x-data="{ filter:'all', showProfile:false, showMore:false, showValidate:false }">
        <div class="page" style="padding:0;max-width:none;">
            ...
            <div class="op-toolbar">
                ...
                <span class="op-status {{ $canValidate ? 'ok' : '' }}">
                    {!! $icon($canValidate ? 'checkCircle' : 'alertCircle') !!}
                    @if ($canValidate)
                        All checked
                    @else
                ...
                <div class="op-actions">
                    ...
                    <div class="dropdown" x-data="{open:false}" @click.outside="open=false">
                        ...
                        <div class="menu" x-show="open" x-cloak>
                            @if ($waUrl)
                                <a class="menu-item" href="{{ $waUrl }}" target="_blank" @click="open=false">
                                    {!! $icon('chat') !!}<span class="mi-main"><span class="mi-t">WhatsApp reminder</span><span class="mi-d">Pre-filled message with checklist link</span></span>
                                </a>
                            @else
                                <button class="menu-item" disabled style="opacity:.5;">
                                    {!! $icon('chat') !!}<span class="mi-main"><span class="mi-t">WhatsApp reminder</span><span class="mi-d">Customer phone missing / disabled</span></span>
                                </button>
                            @endif
                            <button class="menu-item" disabled style="opacity:.5;">
                                {!! $icon('mail') !!}<span class="mi-main"><span class="mi-t">Email reminder</span><span class="mi-d">Coming soon</span></span>
                            </button>
                        </div>
                    </div>
                    {{-- Print --}}
                    <div class="dropdown" x-data="{open:false}" @click.outside="open=false">
                        ...
                        <div class="menu" x-show="open" x-cloak>
                            <button class="menu-item" wire:click="downloadChecklist" @click="open=false">
                                {!! $icon('list') !!}<span class="mi-main"><span class="mi-t">Checklist form</span><span class="mi-d">Item-by-item condition checklist</span></span>
                            </button>
                            <button class="menu-item" wire:click="downloadDeliveryNote" @click="open=false">
                                {!! $icon('truck') !!}<span class="mi-main"><span class="mi-t">Delivery note</span><span class="mi-d">Hand-over / delivery document</span></span>
                            </button>
                        </div>
                    </div>
                    <a class="btn" href="{{ $this->deliveryDocsUrl() }}">{!! $icon('doc') !!}Delivery</a>
                    <a class="btn" href="{{ $this->editRentalUrl() }}">{!! $icon('edit') !!}Edit Rental</a>
                    <button class="btn btn-success" @click="showValidate=true" @disabled(! $canValidate)>{!! $icon('truck') !!}Validate Pickup</button>
                </div>
            </div>
            ...
            <div class="card rental-card">
                <div class="rental-id">
                    ...
                    <span class="badge b-warning"><span class="dot"></span>{{ ucfirst(str_replace('_', ' ', $rental->status)) }}</span>
                </div>
                <div class="rental-stats">
                    <div class="rs-cell">
                        <span class="rs-k">{!! $icon('truck') !!}Pickup</span>
                        <span class="rs-v">{{ $rental->start_date?->format('d M Y · H:i') }}</span>
                    </div>
                    <div class="rs-cell">
                        <span class="rs-k">{!! $icon('calendar') !!}Due back</span>
                        <span class="rs-v">{{ $rental->end_date?->format('d M Y · H:i') }}</span>
                    </div>
                    <div class="rs-cell">
            ...
            @if ($conflictCount)
                <div class="conflict-banner">
                    <div class="cb-head">
                        ...
                        <div class="cb-headtx">
                            <strong>{{ $conflictCount }} unit{{ $conflictCount > 1 ? 's' : '' }} unavailable</strong>
                            <span>Resolve each conflict before validating pickup.</span>
                        </div>
                        <span class="cb-count">{{ $conflictCount }}</span>
                    </div>
                    <div class="cb-list">
            ...
            <div class="card">
                <div class="card-head">
                    <h2>{!! $icon('list') !!}Pickup Checklist</h2>
                </div>
            ...
            <div class="scrim" wire:click.self="closeEditor">
                <div class="modal" wire:key="editor-{{ $editingId }}">
                    <div class="modal-head">
                        ...
                        <p>{{ $editSn }} · Record condition on pickup</p>
                    </div>
                    <div class="modal-body">
                        <div class="field full">
                            <span class="field-label">Condition (on pickup)</span>
                            <div class="cond-seg">
        ...
        {{-- Swap modal --}}
        @if ($swapId)
            @php $cands = $this->swapCandidates(); @endphp
            <div class="scrim" wire:click.self="closeSwap">
                <div class="modal">
                    <div class="modal-head">
                        <h3>{!! $icon('swap') !!}Swap unit</h3>
                        <p>Replace with an available unit of the same product. Its kits will be re-attached automatically.</p>
                    </div>
                    <div class="modal-body">
                        @if (count($cands))
                            <div class="radio-cards">
                                @foreach ($cands as $c)
                                    <button class="radio-card" aria-pressed="{{ $swapUnitId == $c['id'] ? 'true' : 'false' }}" wire:click="$set('swapUnitId', '{{ $c['id'] }}')">
                                        <span class="rc-dot"></span>
                                        <span class="rc-main"><span class="rc-t mono">{{ $c['serial'] }}</span><span class="rc-d">{{ $c['status'] }} · same product</span></span>
                                        <span class="badge b-success"><span class="dot"></span>Available</span>
                                    </button>
                                @endforeach
                            </div>
                        @else
                            <div class="banner banner-warning">{!! $icon('alert') !!}<div class="body"><strong>No available units.</strong> There are no free units of this product for the rental period.</div></div>
                        @endif
                    </div>
                    <div class="modal-foot">
                        <span style="flex:1"></span>
                        <button class="btn" wire:click="closeSwap">Cancel</button>
                        <button class="btn btn-primary" wire:click="confirmSwap" @disabled(! $swapUnitId)>{!! $icon('swap') !!}Confirm swap</button>
                    </div>
                </div>
            </div>
        @endif
        ...
        <template x-if="showValidate">
            <div class="scrim" @click.self="showValidate=false" @keydown.escape.window="showValidate=false">
                <div class="modal">
                    <div class="modal-head">
                        <h3>{!! $icon('truck') !!}Confirm pickup</h3>
                        <p>This will mark the delivery as out and change the rental status to Active.</p>
                    </div>
                    <div class="modal-body">
                        <div class="sum-row"><span class="sk">{!! $icon('layers') !!}Items handed over</span><span class="sv">{{ $checkedCount }} of {{ $total }}</span></div>
                        @if ($damagedItems->count())
                            <div class="banner banner-warning">{!! $icon('alert') !!}<div class="body"><strong>{{ $damagedItems->count() }} item(s) flagged with issues on the way out.</strong> Condition is recorded as the baseline for return.</div></div>
                        @endif
                        @if (! $availability['available'])
                            <div class="banner banner-danger">{!! $icon('xCircle') !!}<div class="body"><strong>Cannot proceed.</strong> Some units are unavailable or double-booked. Swap them first.</div></div>
                        @else
                            <div class="banner banner-success">{!! $icon('checkCircle') !!}<div class="body">No scheduling conflicts. All clear to hand over.</div></div>
                        @endif
                    </div>
                    <div class="modal-foot">
                        <span style="flex:1"></span>
                    ...
                    <div class="modal-head">
                        <div class="cust-head">
                            ...
                            <div class="cust-id">
                                ...
                                <div class="cust-tags"><span class="badge b-warning"><span class="dot"></span>{{ ucfirst(str_replace('_', ' ', $rental->status)) }}</span></div>
        ...
        <div class="mobile-actionbar">
            ...
            <button class="btn btn-success mab-primary" @click="showValidate=true" @disabled(! $canValidate)>{!! $icon('truck') !!}Validate Pickup</button>
        </div>
        ...
        <template x-if="showMore">
            <div class="scrim" @click.self="showMore=false" @keydown.escape.window="showMore=false">
                <div class="modal">
                    <div class="modal-head"><h3>{!! $icon('bolt') !!}Operation actions</h3><p>Rental {{ $rental->rental_code }} · Pickup</p></div>
                    <div class="modal-body">
                        <div class="act-sheet">
                            <div class="act-group">
                                <div class="act-label">{!! $icon('chat') !!}Send to customer</div>
                                @if ($waUrl)
                                    <a class="act-item" href="{{ $waUrl }}" target="_blank" @click="showMore=false"><span class="act-ic">{!! $icon('chat') !!}</span><span class="act-tx"><b>WhatsApp reminder</b><small>Pre-filled message with checklist link</small></span>{!! $icon('arrowRight', 'act-chev') !!}</a>
                                @endif
                            </div>
                            <div class="act-group">
                                ...
                                <button class="act-item" wire:click="downloadDeliveryNote" @click="showMore=false"><span class="act-ic">{!! $icon('truck') !!}</span><span class="act-tx"><b>Delivery note</b><small>Hand-over / delivery document</small></span>{!! $icon('arrowRight', 'act-chev') !!}</button>
                            </div>
                            <div class="act-group">
                                <a class="act-item" href="{{ $this->editRentalUrl() }}"><span class="act-ic">{!! $icon('edit') !!}</span><span class="act-tx"><b>Edit Rental</b><small>Adjust items, dates, deposit &amp; pricing</small></span>{!! $icon('arrowRight', 'act-chev') !!}</a>
